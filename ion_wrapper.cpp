...
int ion_alloc_fd(int fd, size_t len, size_t align, unsigned int alloc_hmask, unsigned int flags,
                 int* handle_fd) {
    ion_user_handle_t handle;
    int ret;

    ///// find the heap mask of new ion device;
    unsigned int heap_mask = 0;
    int num_heaps = 0;

    if (ion_is_legacy(fd)) {
        heap_mask = alloc_hmask;
    } else if (ion_query_heap_cnt(fd, &num_heaps) >= 0) {

        struct ion_heap_data *const heaps = (struct ion_heap_data *)malloc(num_heaps *
                sizeof(struct ion_heap_data));
        if (heaps != NULL)
        {
            if (ion_query_get_heaps(fd, num_heaps, heaps) >= 0)
            {
                for (int i = 0; i != num_heaps; ++i)
                {
                    //ALOGD("%d, type=%d, alloc_hmask=%x\n", i, heaps[i].type, alloc_hmask);
...
int ion_query_get_heaps(int fd, int cnt, void* buffers) {
    int ret;
    struct ion_heap_query query = {
        .cnt = (__u32)cnt, .heaps = (uintptr_t)buffers,
    };
